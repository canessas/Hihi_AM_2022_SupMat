# NIMBLE state-space model for estimation ------------------------------
hihiCode <- nimbleCode({
...
# Decision rules for alternative strategies ------------------------------
...
strategy.1.rules <<- function(yr=y,ns=n.sites,sn=site.names,st=site.type,sa=site.ages,sx=site.access,fl=feed.levels,
  ...
  # Genetic releases - every fifth year harvest Hauturu and allocate to the two occupied sites with the longest time since last genetic imports
  if(((yr+2)%%5)==0){ # Plan two years before!
    gg[which(sa==sort(sa[sn!="Hauturu"&N.est[yr+1,]>1&colSums(Gx[1:(yr+1),])==min(colSums(Gx[1:(yr+1),]))])[1])] <- 1
    gg[which(sa==sort(sa[sn!="Hauturu"&N.est[yr+1,]>1&colSums(Gx[1:(yr+1),])==min(colSums(Gx[1:(yr+1),]))])[2])] <- 1 # If there is a second site
...
while(sum(rr>0)<sum(hh)|j<max.rr){ # If number of release sites matches that of potential sources, break
  ...
  rr[which(N.est[yr+1,]/Ke[yr+1,,fl]==sort(N.est[yr+1,cond.4]/Ke[yr+1,cond.4,fl])[j])] <- max(rr)+1
  if(sum(rr>0)<sum(hh)|j<max.rr)break
  # FIFTH PRIORITY: rank by N/K.unfed
  rr[which(N.est[yr+1,]/Ke[yr+1,,1]==sort(N.est[yr+1,cond.5]/Ke[yr+1,cond.5,1])[j])] <- max(rr)+1
  if(sum(rr>0)<sum(hh)|j<max.rr)break
...
strategy.3.rules <<- function(yr=y,ns=n.sites,sn=site.names,st=site.type,sa=site.ages,sx=site.access,fl=feed.levels,
  ...
  j=max(rr)+1
  while(sum(rr>0)<sum(hh)){ # If number of release sites matches that of potential sources, break
    rr[which(N.est[yr+1,]/Ke[yr+1,,1]==sort(N.est[yr+1,cond.3]/Ke[yr+1,cond.3,1])[j-max(rr)])] <- j
...
strategy.4.rules <<- function(yr=y,ns=n.sites,sn=site.names,st=site.type,sa=site.ages,sx=site.access,fl=feed.levels,
  ...
  j=max(rr)+1
  while(sum(rr>0)<sum(hh)){ # If number of release sites matches that of potential sources, break
    rr[which(N.est[yr+1,]/Ke[yr+1,,fl]==sort(N.est[yr+1,cond.3]/Ke[yr+1,cond.3,fl])[j-max(rr)])] <- j
    if(sum(rr>0)<sum(hh)|sum(rr>0)==length(cond.3)){break} # When all sites with this priority level have been used, break
    j=j+1 # If there are still sites available with this level of priority, another round
  }
  ...
  j=max(rr)+1
  while(sum(rr>0)<sum(hh)){ # If number of release sites matches that of potential sources, break
    rr[which(N.est[yr+1,]/Ke[yr+1,,1]==sort(N.est[yr+1,cond.4]/Ke[yr+1,cond.4,1])[j-max(rr)])] <- j
...
#### Calculate mode for pert distributions ####
Mode <- function(x) {
  ux <- unique(x)
...
#### Calculate cost of translocations in a given year ####
trans.cost <- function(Harvest,trans.mat,site.names,site.access){
  
  if(sum(Harvest>0,na.rm=T)==0|sum(Harvest)!=0){out <- 0}else{
     
    # Storage vectors
    st <- Harvest
    site.comb <- data.frame(Source=character(),Destination=character(),
                            Access.s=character(),Access.d=character(),Cost=integer()) 
    # Pair sources and destinations
    # nrow(site.comb)+1
    while(sum(st>0)>0){
      # This combination of source and destination
      site.comb[nrow(site.comb)+1,1:4] <- c(site.names[st<0][1],site.names[st>0][1], # Site names
                                            site.access[st<0][1],site.access[st>0][1]) # Site access
      # Calculate difference between harvest and release
      diff <- st[st<0][1]+st[st>0][1]
      # Discount birds used
      st[st<0][1] <- min(0,diff)
      st[st>0][1] <- max(0,diff)
    }
    # Order by source name and access so remote sources/destinations are always counted first
    site.comb <- site.comb[order(site.comb[,1],-xtfrm(site.comb[,3]),-xtfrm(site.comb[,4])), ]
    # Calculate costs:
    #  - For combinations that include a remote site, always count cost
    #  - For combinations that include accessible sites, aggregate based on source
    for(i in 1:nrow(site.comb)){
      site.comb$Cost[i] <-
        # If source is remote AND has not been counted OR has been counted AND destination is remote, add cost
        ifelse(sum(site.comb$Cost[site.comb$Source==site.comb$Source[i]],na.rm=T)==0|site.comb$Access.d[i]=="Remote",
               trans.mat[site.comb[i,1],site.comb[i,2]],0) # Otherwise included in previous costs
    }
    out <- sum(site.comb$Cost)
  }
  return(out)
}
