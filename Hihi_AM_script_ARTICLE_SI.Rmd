---
title: "Appendix 3 - Simulation script for parallel NIMBLE run"
...
invisible(lapply(c("mc2d","abind","doParallel","doRNG","doSNOW","itertools","nimble","readxl","stringr","tidyverse"), 
library, character.only=TRUE))
# Source strategy decision rules and extra functions
source("Hihi_AM_all_strategies_source_ARTICLE_SI.R",local=TRUE)

# Set number of cores for parallel run
max.cores <- detectCores(logical=FALSE)
n.reps <- 1
# n.sims is a multiple of n.cores: how many simulations per core?
n.sims <- (max.cores-1)*n.reps; n.years <- 3; feed.levels <- 2 # Simulation settings 
ni <- 100; nb <- 50; nt <- 1; nc <- 3 # MCMC settings
# this.seed <- round(runif(1,1,9999))
# set.seed(this.seed)
...
final.K <- read_excel("Hihi_AM_priors_ARTICLE_SI.xlsx", sheet = "Hihi_AM_elicited_K")
...
K.sites.orig[2,"Hauturu",] <- K.sites.orig[1,"Hauturu",] # Hauturu
K.sites.orig[2,"Cuvier",] <- K.sites.orig[1,"Cuvier",] # Cuvier
K.sites.orig[2,"Tara",] <- K.sites.orig[1,"Tara",] # Taranga
# Fix Rotoroa unfed - avoid the all-zero distribution which sometimes leads to NIMBLE failure
...
# set.seed(this.seed) # Change or fix for stochasticity among strategies
  ...
  full.results <- foreach(s=1:n.sims,.inorder = TRUE,
                          ...
                          .packages=c("abind","mc2d","nimble","readxl","stringr","tidyverse")) %dorng% {
    ...
    post <- read_excel("Hihi_AM_priors_ARTICLE_SI.xlsx", sheet = "Hihi_AM_posteriors_2020")
...
# Load matrix of pairwise translocation costs
trans.mat <- as.data.frame(read_excel("Hihi_AM_priors_ARTICLE_SI.xlsx", sheet = "Hihi_AM_translocation_costs")[,-1])
row.names(trans.mat) <- as.data.frame(read_excel("Hihi_AM_priors_ARTICLE_SI.xlsx", sheet = "Hihi_AM_translocation_costs"))[,1]
# Fill the lower diagonal of the matrix
for(i in 1:nrow(trans.mat)) {for(j in 1:i) {trans.mat[i,j]=trans.mat[j,i] }}
trans.mat <- trans.mat[site.names,site.names] # Re-order to match original site names

site.Costs <- open.Costs <- array(0, dim=dim(all.N))
colnames(site.Costs) <- colnames(open.Costs) <- site.names
trans.Costs <- array(0, dim=dim(all.N)[c(1,3,4)])
for(i in 1:dim(all.N)[3]){for(j in 1:dim(all.N)[4]){
  
  # Cost of site maintenance:
  site.Costs[,,i,j] <- 
    # Cost of national ranger: 72800 (not added for now)
    (all.N[,,i,j]>0)*ranger.mat*36400 + # Cost of each occupied site(ranger)
    (all.N[,,i,j]>0&all.FR[,,i,j]==1)*(11492+9100) + # Cost of each fed site - breeding  + off season (volunteer)
    (all.N[,,i,j]>0&all.FR[,,i,j]==1)*ifelse(bellbird.mat==0,128,1920) + # Cost of sugar depending on bellbird presence
    (all.N[,,i,j]>0&all.FR[,,i,j]==1)*vehicle.mat*60 # Cost of vehicle where required
    
  # Cost of translocations
  for(t in 1:dim(trans.Costs)[1]){
    Hv <- all.Harvest[t,,i,j]
    trans.Costs[t,i,j] <- trans.cost(Hv,trans.mat,site.names,site.access)
  }

  # Cost of opening new sites
  open.Costs [,,i,j] <-
    (all.N[,,i,j]==0&all.Harvest[,,i,j]>0)*1600 + # Add nest boxes
    (all.N[,,i,j]==0&all.Harvest[,,i,j]>0&all.FR[,,i,j]==1)*4000 # Add feeding cages if site fed
}}

# Adjust all costs for inflation
NZ.inf <- read_excel("Hihi_AM_priors_ARTICLE_SI.xlsx", sheet = "Hihi_AM_NZ_inflation")
for(i in 1:dim(all.N)[3]){for(j in 1:dim(all.N)[4]){
  # Draw inflation rates randomly based on last n years (different across sims and strategies)
  yr.inf <- sample(NZ.inf$Inflation[NZ.inf$Year>2002],dim(all.N)[1],replace=TRUE)*0.01
  for(t in 2:dim(site.Costs)[1]){
    # Translocation costs
    trans.Costs[t,i,j] <- trans.Costs[t,i,j]*prod(1+yr.inf[2:t])
    # Site maintenance costs
    site.Costs[t,,i,j] <- site.Costs[t,,i,j]*prod(1+yr.inf[2:t])
    # New site costs
    open.Costs[t,,i,j] <- open.Costs[t,,i,j]*prod(1+yr.inf[2:t])
  }
}}

# Calculate total costs by year
library(abind)
all.Costs <- apply(abind(apply(site.Costs,c(1,3,4),sum),
                   apply(open.Costs,c(1,3,4),sum),
                   trans.Costs, along=4),c(1,2,3),sum)

####################
# Calculate advocacy

NZ.visitors <- read_excel("Hihi_AM_priors_ARTICLE_SI.xlsx", sheet = "Hihi_AM_NZ_visitors")
adv <- filter(NZ.visitors,Year==2019) # Take 2019 to avoid pandemic effects
adv[,"Bushy"] <- filter(NZ.visitors,Year==2020)["Bushy"] # Bushy changed management, so use 2020 values
adv <- adv[-1] # Remove year
adv.cv <- adv*0.05 # 5% annual variability

all.Visitors <- array(NA, dim=dim(all.N))
colnames(all.Visitors) <- site.names
for(i in 1:dim(all.N)[3]){for(j in 1:dim(all.N)[4]){for(t in 1:dim(all.N)[1]){
  all.Visitors[t,,i,j] <-
  (all.N[t,,i,j]>0)*floor(rnorm(n.sites,as.numeric(adv),as.numeric(adv.cv))) # Variable annual N visitors
}}}
...
# load("C:/Users/Stefano/Dropbox/Hihi_AM/Hihi_AM_stats/Hihi_AM_nimble_parallel_results_ALL_OUTCOMES.RData")
# Load packages and functions
invisible(lapply(c("tidyverse", "ggplot2"), library, character.only=TRUE))
# Source strategy decision rules and extra functions
source("Hihi_AM_all_strategies_source_ARTICLE_SI.R",local=TRUE)

n.sims <- dim(all.N)[3] # Check dimensions are correct
run.strategies <- 1:dim(all.N)[4]
...
# Aggregate results for all objectives
all.outcomes <- rbind(
  # Total final population size for each strategy (not counting Hauturu)
  data.frame(
    Strategy=rep(strategy.names,each=n.sims),
    Sim=rep(1:n.sims,length(run.strategies)),
    Variable=rep("Number",n.sims*length(run.strategies)),
  ...
  data.frame(
    Strategy=rep(strategy.names,each=n.sims),
    Sim=rep(1:n.sims,length(run.strategies)),
    Variable=rep("Populations",n.sims*length(run.strategies)),
  ...
  data.frame(
    Strategy=rep(strategy.names,each=n.sims),
    Sim=rep(1:n.sims,length(run.strategies)),
    Variable=rep("Cost",n.sims*length(run.strategies)),
    Value=as.numeric(apply(all.Costs/1000,c(2,3),sum))),
  
  # Total number of feeding interventions for each strategy
  data.frame(
    Strategy=rep(strategy.names,each=n.sims),
    Sim=rep(1:n.sims,length(run.strategies)),
    Variable=rep("Feeding",n.sims*length(run.strategies)),
    Value=as.numeric(apply(all.FR,c(3,4),sum))),
  
  # Total number of site visitors for each strategy
  data.frame(
    Strategy=rep(strategy.names,each=n.sims),
    Sim=rep(1:n.sims,length(run.strategies)),
    Variable=rep("Visitors",n.sims*length(run.strategies)),
    Value=as.numeric(apply(all.Visitors/1000,c(3,4),sum))) 
)
all.outcomes$Variable <- factor(all.outcomes$Variable,levels=unique(all.outcomes$Variable))

# Rescaling functions
rescale.max <- function(x){if((max(x)-min(x))==0){1}else{(x - min(x))/(max(x)-min(x))}} # Rescale function #1 (maximise); if all the same, all 1
rescale.min <- function(x){if((max(x)-min(x))==0){0}else{(max(x) - x)/(max(x)-min(x))}} # Rescale function #2 (minimise); if all the same, all 0

# Load swing weights for the three scenarios
base.wts <- read_excel("Hihi_AM_priors_ARTICLE_SI.xlsx", sheet = "Hihi_AM_weights")
# Get factor in the right order
base.wts$Scenario <- factor(base.wts$Scenario,levels=c("Worst","Average","Best"))

# Add empty vector for normalized weights
base.wts$Normalized <- rep(NA,nrow(base.wts))
# Normalize weights by participant
for(s in 1:length(unique(base.wts$Scenario))){for(p in 1:length(unique(base.wts$Participant))){
  wps <- base.wts$Weights[base.wts$Scenario==unique(base.wts$Scenario)[s]&
                            base.wts$Participant==unique(base.wts$Participant)[p]]
  base.wts$Normalized[base.wts$Scenario==unique(base.wts$Scenario)[s]&
                        base.wts$Participant==unique(base.wts$Participant)[p]] <- wps/sum(wps)
}}

# Create array: consequence table x participant x scenario
smart.array <- array(NA,dim=c(length(strategy.names),length(unique(base.wts$Objective)),n.sims),
                     dimnames=list(strategy.names,unique(base.wts$Objective),1:n.sims))
first.choice <- rep(NA,n.sims)
# Fill the SMART array
for(s in 1:dim(smart.array)[3]){
  # Create consequence table for this scenario
  s.temp <- reshape(all.outcomes[all.outcomes$Sim==s,], idvar = c('Sim','Strategy'), timevar = 'Variable', direction = "wide")
  colnames(s.temp) <- gsub("Value.", "",colnames(s.temp))
  # Rescale consequence table for this scenario
  s.temp[,c(3,4,7)] <- apply(s.temp[,c(3,4,7)],2,rescale.max) # Variables to maximize
  s.temp[,5:6] <- apply(s.temp[,5:6],2,rescale.min) # Variables to minimize
  # Apply weights for each participant for this scenario
  for(p in 1:dim(smart.array)[3]){
    # Get the weights for this participant and scenario (in same order as df columns)
    w.temp <- tapply(base.wts$Normalized,list(base.wts$Scenario,base.wts$Objective),mean)['Best',][colnames(s.temp)[3:7]]
    # Apply weight 
    smart.array[,,s] <- matrix(as.numeric(unlist(sweep(s.temp[,3:7],MARGIN=2,w.temp,'*'))),
                                 nrow=dim(smart.array)[1],ncol=dim(smart.array)[2])
    # Find highest-ranking strategy
    first.choice[s] <- which.max(rowSums(smart.array[,,s]))
  }
}
...
rowMeans(apply(smart.array,c(1,3),sum))
# How often is each strategy the first choice?
table(first.choice)
